#!/usr/bin/env python3
"""
Test the REAL Paystack renewal path end-to-end for Founder Basic (monthly) --
...
Sibling scripts test_renewal_investor.py / test_renewal_connector.py cover
the other two billable roles -- each finalizes through a different function
(finalize_pro_subscription / finalize_investor_subscription /
finalize_connector_subscription) so all three need independent coverage.

Run on KVM2 (dev environment only -- resets the target test account):
  set -a && source /root/.env.dev && set +a && python3 scripts/test_renewal_founder.py
"""
...
TEST_EMAIL    = "kevin.metatron.testing+founderbasic@gmail.com"
...
# ── 2. Subscribe (founder_basic / monthly) ───────────────────
step("2. POST /commerce/subscribe (founder_basic / monthly / ZAR)")
r = requests.post(f"{BACKEND}/commerce/subscribe",
    headers={"Authorization": f"Bearer {token}"},
    json={"tier": "founder_basic", "billing": "monthly", "currency": "ZAR"})
...
tx_data = r.json()["data"]
tx_data["metadata"] = {"user_id": user_id, "tier": "founder_basic", "billing": "monthly", "currency": "ZAR"}
...
plan_code = os.getenv("PAYSTACK_FOUNDER_BASIC_MONTHLY", "")
