#!/usr/bin/env python3
"""
Test the REAL Paystack renewal path end-to-end for Connector (monthly) --
...
Run on KVM2:
  set -a && source /root/.env.dev && set +a && python3 scripts/test_renewal_connector.py
"""
...
TEST_EMAIL    = "kevin.metatron.testing+connector@gmail.com"
...
# ── 2. Subscribe (connector basic / monthly) ──────────────────
step("2. POST /commerce/connector/subscribe (monthly)")
r = requests.post(f"{BACKEND}/commerce/connector/subscribe",
...
tx_data = r.json()["data"]
tx_data["metadata"] = {"user_id": user_id, "tier": "connector_basic", "billing": "monthly", "currency": "ZAR"}
...
plan_code = os.getenv("PAYSTACK_CONNECTOR_PLAN_BASIC_MONTHLY", "")
