...
export interface MonthlySummaryMatchCard {
  id: string;
...
interface FounderMonthlySummaryProps {
  monthName: string;
...
export default function FounderMonthlySummary({
  monthName = "May 2026",
  totalThisMonth = 4,
  tier = "basic",
  matches = [],
      ...
      <Body style={body}>
        <Container style={container}>
          ...
          <Text style={statsText}>
            Kevin found{" "}
            <span style={statsHighlight}>{totalThisMonth} investor matches</span>{" "}
          ...
          {matches.map((m) => (
            <Section key={m.id} style={card}>
              <Section style={cardHeader}>
                <Text style={firmName}>{m.firm_name || "Investor"}</Text>
          ...
          {isFree && (
            <Section style={upgradeSection}>
              <Text style={upgradeText}>
                Upgrade to basic to get 10 weekly matches and full Angel Score
                details every month.
              </Text>
              <Link
